 ...
 * 探活接口（供 agent 调用）
 *
 * 用途：判断当前 auth-key 对应的微信 token 是否仍然有效，不返回任何微信内部数据
 *
 * 鉴权：X-Internal-Key === DEBUG_KEY（管理鉴权，不依赖 token 本身）
 *
...
export default defineEventHandler(async event => {
  ...
  // 即使 query 里传假 token 也无所谓：proxyMpRequest 会从 event 上真实 cookie 取出 session
